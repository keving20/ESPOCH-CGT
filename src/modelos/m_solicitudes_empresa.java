 ...
 * @author USER
 */
public class m_solicitudes_empresa {

    //////////////////////////////////////////////////////////////
    //para  llenar en memoria los datos que provienen de la BD hacemos lo siguiente
    public static ArrayList<solicitudes_empresa> llenarSolicitudesEmpresa(ConjuntoResultado rs) throws Exception {
        ArrayList<solicitudes_empresa> lst = new ArrayList<>();
        solicitudes_empresa sol_empresa = null;
        try {
            while (rs.next()) {

                sol_empresa = new solicitudes_empresa(rs.getInt(0), rs.getString(1), rs.getString(2), rs.getString(3), rs.getBigDecimal(4),rs.getString(5),  rs.getString(6), rs.getString(7), rs.getString(8), rs.getString(9), rs.getInt(10), rs.getString(11), rs.getString(12), rs.getString(13), rs.getString(14), rs.getString(15), rs.getString(16));
                lst.add(sol_empresa);
            }
        } catch (Exception e) {
            lst.clear();
 ...
 ///////seleccionar las solicitudes de un trabjador por un id
    public static ArrayList<solicitudes_empresa> obtenerSolicitudesEmpresa(int id_empresa) throws Exception {
        ArrayList<solicitudes_empresa> lst = new ArrayList<>();
        ArrayList<Parametro> lstP = new ArrayList<>();
        try {
            String sql = "select * from public.f_select_solicitud_empresa(?)";
            lstP.add(new Parametro(1, id_empresa));

            ConjuntoResultado rs = AccesoDatos.ejecutaQuery(sql, lstP);
            lst = llenarSolicitudesEmpresa(rs);
            rs = null;
        } catch (SQLException exConec) {
            throw new Exception(exConec.getMessage());
        }
    ...
    public static boolean eliminarSolicitud(int idSolicitud) throws Exception {
        ...
        try {
            ...
            String sql = "select * from public.f_delete_solicitud_empresa(?)";
